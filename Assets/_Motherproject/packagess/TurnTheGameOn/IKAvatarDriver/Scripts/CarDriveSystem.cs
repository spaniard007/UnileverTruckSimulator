...
#pragma warning restore 0067
        ...
        public void Move(float steering, float accel, float footbrake, float handbrake)
        {
            ...
            UpdateGear();
            if (vehicleSettings.enableDownForce) _rigidbody.AddForce(-_transform.up * _vehicleSettings.downforce * _rigidbody.linearVelocity.magnitude); // this is used to add more grip in relation to speed
            TractionControl();
            ...
            velocityMagnitude = distanceTraveledFromPreviousPosition == 0 ? 0 : _rigidbody.linearVelocity.magnitude;
        ...
        private void CapSpeed()
        {
            ...
            if (currentSpeed > _vehicleSettings.topSpeed) _rigidbody.linearVelocity = (_vehicleSettings.topSpeed / speedMultiplier) * _rigidbody.linearVelocity.normalized;
        }
        ...
        private void ApplyDrive(float accel, float footbrake)
        {
            ...
            else
            {
                ...
                {
                    if (currentSpeed > 0 && Vector3.Angle(_transform.forward, _rigidbody.linearVelocity) < 50f) ApplyBrake(footbrake);
                    else if (footbrake > 0)
                    {
                        if (currentSpeed < _vehicleSettings.topSpeedReverse) Reverse(footbrake);
                    }
        ...
        private void UpdateWheels(float _steerInput, float _handBrake)
        {
            ...
            if (Mathf.Abs(previousRotationY - _transform.eulerAngles.y) < 10f) // this if is needed to avoid gimbal lock problems that will make the car suddenly shift direction
            {
                ...
                _rigidbody.linearVelocity = steerHelper__velRotation * _rigidbody.linearVelocity;
            }
        ...
        private void DragOverride(float _throttleInput, float _steerinput)
        {
            ...
            {
                _rigidbody.linearDamping = vehicleSettings.minDrag;
                _rigidbody.angularDamping = vehicleSettings.minAngularDrag;
            }

            else // apply max drag when not accelerating
            {
                _rigidbody.linearDamping = vehicleSettings.maxDrag;
                _rigidbody.angularDamping = vehicleSettings.maxAngularDrag;
            }
        }
