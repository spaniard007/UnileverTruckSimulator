...
namespace CiDy
{
    public class CiDyEditor : EditorWindow
    {
        ...
        {
            graph = FindAnyObjectByType<CiDyGraph>();//Grab the Graph
            if (graph != null)
            {
                graphSO = new SerializedObject(graph);
            }
...
            }
        ...
        void BackupScene()
        {
            ...
            Terrain[] allTerrains = FindObjectsByType<Terrain>(FindObjectsSortMode.InstanceID);
...
#endif
            allTerrains = FindObjectsByType<Terrain>(FindObjectsSortMode.InstanceID);
            //Sort Terrains By X and Z Axis, 
            allTerrains = allTerrains.OrderBy(x => x.GetPosition().x).ThenBy(x => x.GetPosition().z).ToArray();
                            ...
                            if (e.type == EventType.MouseDown)
                            {
                                ...
                                if (Physics.Raycast(worldRay, out hit, Mathf.Infinity, nodeMask))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (curData != null && !(Keyboard.current.leftCtrlKey.isPressed || Keyboard.current.rightCtrlKey.isPressed))
                                        {
